...
This is the production interface — what FastAPI calls.
...
def _load_lookup_data():
    """
    Load and cache everything needed for inference:
    - customer cluster assignments
    - per-customer category affinity
    - coupon catalog
    - cluster-level category preferences
    - p99 normalization constants (for log-scaled scoring)
    """
    ...
    # Population-level normalization constants — must match what the
    # batch ranker uses, otherwise inference scores would be on a
    # different scale than training-time scores.
    ...
    return customer_clusters, affinity, catalog, cluster_prefs, affinity_p99, cluster_p99
...
def get_coupons(customer_token: str, top_n: int = 3) -> List[Dict]:
    ...
    """
    (customer_clusters, affinity, catalog, cluster_prefs,
     affinity_p99, cluster_p99) = _load_lookup_data()
